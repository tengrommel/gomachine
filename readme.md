...
# 第三章评估与验证
>为了拥有可持续的，负责任的机器学习工作流程并开发产生真正价值的机器学习应用程序，我们需要能够衡量我们的机器学习模型的性能。我们还需要确保我们的机器学习模型能够推广到他们将在生产中看到的数据。如果我们不做这些事情，那么我们基本上就是在黑暗中射击。我们将不了解我们模型的预期行为，并且随着时间的推移我们将无法对其进行改进。

测量模型如何执行（相对于某些数据）的过程称为评估。确保我们的模型推广到我们可能期望遇到的数据的过程称为验证。

# 评估
> 科学的基本原则是测量，而机器学习科学也不例外。我们需要能够测量或评估我们的模型的性能，因此我们可以继续对其进行改进，将一个模型与另一个模型进行比较，并检测模型何时表现不佳。

只有一个问题。我们如何评估模型的性能？我们是否应该衡量他们可以被训练或推理的速度？我们应该衡量他们得到正确答案的次数吗？我们如何知道正确的答案是什么？我们应该测量偏离观测值多远吗？我们如何测量该距离？

*如您所见，围绕我们如何评估模型有许多决策。真正重要的是上下文。在某些情况下，效率绝对很重要，但是每个机器学习环境都要求我们衡量我们的预测，推断或结果如何与理想的预测，推断或结果相匹配。因此，测量计算结果与理想结果之间的比较应该始终优先于速度优化。*

# 连续值
> 但是，单独查看所有错误值并不是真正可行或实际的，尤其是在有大量数据的情况下。这些错误值可能有一百万或更多。因此，我们需要一种理解汇总错误的方法。
...
# 验证
> 实际上，可以创建一个超级复杂的模型，用于预测每一个观察结果而不会出错。但是它无法推广到新数据。这种情况称之为过渡拟合(overfit)。

# 训练和测试集
> 防止过渡拟合的第一种方法是在一部分数据集上训练或拟合模型，然后在数据集的另一部分测试或评估该模型。

# 保留集
> 保留集绝不能用于训练和测试

# 第二部分 机器学习
>在本节中，我们将学习各种类型的机器学习技术，并使用Go亲自实践每种技术的示例
...
# 第四章 回归
> 回归是一个过程，通过回归可以了解到一个变量如何随着另一个变量发生变化。

# 术语

- 响应或者因变量
> 这些术语等价于基于一个或多个变量进行预测的另一个变量
...
- 非线性回归
> 这种类型的回归假定因变量非线性依赖自变量

- 多元回归
> 具有多个变量的回归
 
- 拟合和训练
> 参数化一个模型的过程，用以预测特定因变量
